...
const Connection = () => {
    ...
    const [account, setAccount] = useState('0x47114567565467534')

    return (
        <div className="bg-white-800">
            <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-x-5 hidden sm:block sm:ml-6">
            0x471...534
                {/* <NftError errorMessage={errorMessage} /> 
                {/*
                    account
                        ? <h1 className="text-x1 font-bold text-purple-400">{"<" + account.slice(0,4) + "......." + account.slice(-4) + ">"}</h1>
                        : <h1 className="text-x1 font-bold text-red-400">{"Not Connected"}</h1>   
                } 
            </div>
        </div>
    )
